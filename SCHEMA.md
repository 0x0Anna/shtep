...
| `LatAccel_mps2`   | m/s²   | ISO seat-frame lateral (sway). **Renamed from `LatAccel_g` 2026-09-15** - the earlier name claimed g but a 2026-08-07 GT7 circuit-session review (see `PI_TOOLBOX_EXPORT.md` "The accelerometer unit bug") confirmed the values were m/s² all along (±23 is impossible as g, exactly right - ±2.3g - as m/s²): only the suffix was ever wrong, not the data. Do not apply a ×9.81 conversion. |
| `LongAccel_mps2`  | m/s²   | ISO seat-frame longitudinal (surge). Same rename/reasoning as `LatAccel_mps2` above. An earlier 2026-08-04 correlation check against the speed derivative found the right relationship (r=-0.986) but flagged an apparent scale/sign mismatch on that session; the 2026-08-07 review superseded that as a real-world false alarm (per-session derivative noise, not a data bug) - see `PI_TOOLBOX_EXPORT.md` for the resolved conclusion. |
| `VertAccel_mps2`  | m/s²   | ISO seat-frame vertical (heave). Same rename/reasoning as `LatAccel_mps2` above. |
